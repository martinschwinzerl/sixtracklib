...
int main()
{
    ...
    typedef NS(DriftExact)          drift_exact_t;
    ...
    {
        drift_exact_t* drift = SIXTRL_NULLPTR;
        ...
        drift = NS(DriftExact_add)( buffer, length );
    ...
    if( success == 0 )
    {
        FILE* fp = fopen( NS(PATH_TO_TEST_TRACKING_BE_DRIFTEXACT_DATA), "wb" );
...
/* end: tests/testdata/generators/generate_track_be_driftexact_data.c */
