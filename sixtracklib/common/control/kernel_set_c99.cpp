...
::NS(arch_size_t) NS(KernelSet_get_num_of_pinned_purposes)(
    const ::NS(KernelSetBase) *const SIXTRL_RESTRICT set )
{
    return ( set != nullptr ) ? set->numPinnedPurposes() : st::arch_size_t{ 0 };
}
...
{
    return ( set != nullptr ) ? set->ptrKernelConfigBase( purpose ) : nullptr;
}
...
::NS(KernelConfigKey) const* NS(KernelSet_get_ptr_const_current_config_key)(
    const ::NS(KernelSetBase) *const SIXTRL_RESTRICT set )
{
    ::NS(KernelConfigKey) const* ptr = nullptr;

    if( ( set != nullptr ) && ( set->lockable() != nullptr ) )
    {
        _set_t::lock_t const lock( *set->lockable() );
        ptr = &set->currentKernelConfigKey( lock );
    }
...
{
    return ( set != nullptr )
        ? set->kernelConfigId( purpose ) : _set_t::ILLEGAL_KERNEL_CONFIG_ID;
}

bool NS(KernelSet_has_kernel_config_id)(
    const ::NS(KernelSetBase) *const SIXTRL_RESTRICT set,
    ::NS(ctrl_kernel_id_t) const config_id )
{
    return ( ( set != nullptr ) && ( set->hasKernelConfigId( config_id ) ) );
}

bool NS(KernelSet_is_purpose_associated_with_kernel_config_id)(
    const ::NS(KernelSetBase) *const SIXTRL_RESTRICT set,
    ::NS(ctrl_kernel_id_t) const cid,
    ::NS(kernel_purpose_t) const purpose )
{
    return ( ( set != nullptr ) &&
             ( set->isPurposeAssociatedWithKernelConfigId( cid, purpose ) ) );
}

::NS(KernelConfigKey) const* NS(KernelSet_get_ptr_const_kernel_config_key)(
    const ::NS(KernelSetBase) *const SIXTRL_RESTRICT set,
    ::NS(kernel_purpose_t) const purpose )
{
    _set_t::lock_t const lock( *set->lockable() );
...
{
    return ( set != nullptr ) ? set->setKernelSetId( kernel_set_id )
...
::NS(arch_status_t) NS(KernelSet_sync_with_store_and_assign_key)(
    ::NS(KernelSetBase)* SIXTRL_RESTRICT set,
    const ::NS(KernelConfigKey) *const SIXTRL_RESTRICT key,
    bool const perform_config_update_if_required )
{
    return ( ( set != nullptr ) && ( key != nullptr ) )
        ? set->syncWithStore( *key, perform_config_update_if_required )
        : st::ARCH_STATUS_GENERAL_FAILURE;
}
