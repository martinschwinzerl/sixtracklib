...
SIXTRL_STATIC void NS( MemPool_set_capacity )(
    NS( MemPool ) * SIXTRL_RESTRICT pool, SIXTRL_UINT64_TYPE const new_capacity );
...
SIXTRL_STATIC void NS( MemPool_set_begin_offset )(
    NS( MemPool )* SIXTRL_RESTRICT pool,
    SIXTRL_UINT64_TYPE const begin_offset );
...
{
    static size_t const ZERO = (SIXTRL_UINT64_TYPE)0u;
    ...
    if( ( pool != 0 ) && ( chunk_size > ZERO ) && ( begin_alignment > ZERO ) )
    {
        ...
        SIXTRL_UINT64_TYPE requested_capacity = begin_alignment +
            ( ( capacity > chunk_size ) ? capacity : chunk_size );

        SIXTRL_UINT64_TYPE const num_chunks = requested_capacity / chunk_size;
        SIXTRL_UINT64_TYPE const calc_capacity = num_chunks * chunk_size;
        ...
        if( new_buffer != 0 )
        {
            ...
            SIXTRL_UINT64_TYPE const begin_offset = ( addr_mod != ( uintptr_t )0 )
                ? ( SIXTRL_UINT64_TYPE )( begin_alignment - addr_mod ) : ZERO;
    ...
    NS(MemPool)* SIXTRL_RESTRICT pool, SIXTRL_UINT64_TYPE const alignment )
{
    ...
    SIXTRL_STATIC SIXTRL_UINT64_TYPE const ZERO = ( SIXTRL_UINT64_TYPE )0u;

    SIXTRL_UINT64_TYPE const chunk_size =
        NS(MemPool_get_chunk_size)( pool );

    SIXTRL_UINT64_TYPE const buffer_capacity =
    ...
    if( ( alignment > ZERO ) && ( chunk_size > ZERO ) && ( buffer != 0 ) )
    {
        ...
        if( use_align > buffer_capacity )
        {
            ...
            {
                SIXTRL_UINT64_TYPE const offset = use_align - addr_mod;
...
bool NS( MemPool_reserve_aligned )(
    NS( MemPool ) * SIXTRL_RESTRICT pool,
    SIXTRL_UINT64_TYPE const new_capacity, SIXTRL_UINT64_TYPE const align )
{
    bool has_been_changed = false;

    static SIXTRL_UINT64_TYPE ZERO = (SIXTRL_UINT64_TYPE)0u;

    SIXTRL_UINT64_TYPE const current_capacity = NS( MemPool_get_capacity( pool ) );
        ...
        ( chunk_size > ZERO ) )
    {
        ...
        if( NS( MemPool_get_const_buffer( pool ) != 0 ) )
        {
            ...
            if( prev_begin_pos != 0 )
            {
                SIXTRL_UINT64_TYPE const prev_size =
...
void NS( MemPool_set_capacity )( NS( MemPool ) * SIXTRL_RESTRICT pool,
                                 SIXTRL_UINT64_TYPE const new_capacity )
{
    assert( pool != 0 );
