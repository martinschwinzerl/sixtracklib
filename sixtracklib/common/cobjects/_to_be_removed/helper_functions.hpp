...
namespace SIXTRL_CXX_NAMESPACE
{
    ...
    ) SIXTRL_NOEXCEPT
    {
        ...
        if( ( dest_begin != nullptr ) &&
            ( max_dest_begin_values > st::cobj_size_type{ 0 } ) )
        {
            if( ( num_second_values == st::cobj_size_type{ 0 } ) ||
            ...
            {
                st::cobj_size_type const num_first_values =
                    ( max_dest_begin_values > num_second_values )
                        ? max_dest_begin_values - num_second_values
                        : st::cobj_size_type{ 0 };

                st::cobj_size_type ii = st::cobj_size_type{ 0 };
    ...
    CObjElem_init_array_as_copy_fill_remainder(
        ...
        cobj_size_type const num_orig_values,
