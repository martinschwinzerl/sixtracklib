    ...
    #include "sixtracklib/python_bindings.hpp"
...
namespace python
{
    namespace
    {
        ...
        using stl_vars_type = stpy::SixTrackLibModuleVars;
    }

    SixTrackLibCommonCObjectsSubVars::SixTrackLibCommonCObjectsSubVars(
        stpy::SixTrackLibModuleVars& vars ) :
//         c99_cbuffer_type( vars.m_module,
//             stl_vars_type::C99_NAMESPACE_STR( "CBuffer" ) ),
        cobj_index_type( vars.m_module,
            stl_vars_type::C99_NAMESPACE_STR( "CObjIndex" ).c_str() ),
        cobj_garbage_range_type( vars.m_module,
            stl_vars_type::C99_NAMESPACE_STR( "CObjGarbageRange" ).c_str() )
//         ,
//         cbuffer_view_type( vars.m_module, "CBufferView", c99_cbuffer_type ),
//         cbuffer_type( vars.m_module, "CBuffer", cbuffer_view_type )
    {

    }

    void SixTrackLibCommonCObjectsSubVars::init( stl_vars_type& vars )
    {
        this->init_cobj_index_type( vars );
        this->init_cobj_garbage_range_type( vars );
    }

    void SixTrackLibCommonCObjectsSubVars::init_cobj_index_type(
        stl_vars_type& SIXTRL_RESTRICT_REF vars )
    {
        this->cobj_index_type.def( py::init<>() )
            .def_readwrite( "begin_addr", &::NS(CObjIndex)::begin_addr )
            .def_readwrite( "type_id", &::NS(CObjIndex)::type_id )
            .def_readwrite( "size", &::NS(CObjIndex)::size );
    }

    void SixTrackLibCommonCObjectsSubVars::init_cobj_garbage_range_type(
        stl_vars_type& SIXTRL_RESTRICT_REF vars )
    {
        this->cobj_garbage_range_type.def( py::init<>() )
            .def_readwrite( "begin_addr", &::NS(CObjGarbageRange)::begin_addr )
            .def_readwrite( "size", &::NS(CObjGarbageRange)::size );
    }
