#ifndef SIXTRACKLIB_TESTLIB_COMMON_TRACK_TEST_CMP_SIXTRACK_H__
#define SIXTRACKLIB_TESTLIB_COMMON_TRACK_TEST_CMP_SIXTRACK_H__
    ...
    #include "sixtracklib/testlib/common/particle.h"
...
NS(Track_sixtrack_sequ_by_sequ_cobj_flat_buffer)(
...
SIXTRL_STATIC SIXTRL_FN NS(status_type) NS(Track_sixtrack_sequ_by_sequ_cbuffer)(
...
NS(Track_sixtrack_sequ_by_sequ_cbuffer_ext)(
    SIXTRL_CBUFFER_ARGPTR_DEC NS(CBuffer)* SIXTRL_RESTRICT track_pbuffer,
    SIXTRL_CBUFFER_ARGPTR_DEC const NS(CBuffer) *const cmp_pbuffer,
    SIXTRL_CBUFFER_ARGPTR_DEC const NS(CBuffer) *const lattice_buffer,
    SIXTRL_CBUFFER_ARGPTR_DEC NS(CBuffer)* SIXTRL_RESTRICT diff_buffer,
        ...
        (  NS(CObjFlatBuffer_num_objects)( lattice_buffer, slot_size ) > 0u ) )
    {
        ...
        for( ; ii < NUM_SEQUENCES ; ++ii )
        {
            ...
            SIXTRL_PARTICLES_ARGPTR_DEC NS(Particles)* pset_track =
                NS(Particles_cobj_flat_buffer_new)( track_pbuffer,
                    NUM_PARTICLES, slot_size );

            NS(particles_num_type) const NUM_PARTICLES =
                NS(Particles_num_particles)( pset_a );

            SIXTRL_PARTICLES_ARGPTR_DEC diff_pset = ( has_diff_buffer )
                ? NS(Particles_cobj_flat_buffer_new)(
                    diff_buffer, NUM_PARTICLES, slot_size )
                : SIXTRL_NULLPTR;

            NS(particles_num_type) jj = ( NS(particles_num_type) )0u;

            if( ( NUM_PARTICLES <= ( NS(particles_num_type) )0u ) ||
                ( pset_track == SIXTRL_NULLPTR ) ||
                ( ( has_diff_buffer ) &&
                  ( ( diff_pset == SIXTRL_NULLPTR ) ||
            ...
            for( ; jj < NUM_PARTICLES ; ++jj )
            {
                ...
                SIXTRL_PARTICLE_ARGPTR_DEC NS(Particle) diff;

                status |= NS(Particle_clear)( &p );
                status |= NS(Particle_clear)( &pset );
                status |= NS(Particle_clear)( &diff );

                status |= NS(Particles_to_single_particle)( pset_track, jj, &p );
                status |= NS(Praticles_to_single_particle)( pset_b, jj, &cmp_p );
                ...
                start_obj_index = ( NS(cobj_size_type) )(
                    NS(Particle_at_element)( &p ) + 1 );

                end_obj_index = ( NS(cobj_size_type) )(
                    NS(Particle_at_element)( &cmp_p ) + 1 );

                if( ( start_obj_index > end_obj_index ) ||
                    ( end_obj_index > NUM_BEAM_ELEMENTS ) ||
                    ( ( ( NS(cobj_size_type) )SIXTRL_TRACK_ILLEGAL_OBJ_INDEX )
                        <= end_obj_index ) )
                {
                    status = ( NS(status_type) )SIXTRL_STATUS_GENERAL_FAILURE;
                    break;
                ...
                if( has_diff_buffer )
                {
                    SIXTRL_ASSERT( diff_pset != SIXTRL_NULLPTR );
                    status  = NS(Particle_difference)( &cmp_p, &p, &diff );
                ...
                status = NS(Particles_from_single_particle)( pset_track, jj, &p );
                if( status != ( NS(status_type) )SIXTRL_STATUS_SUCCESS ) break;
            }
...
SIXTRL_INLINE NS(status_type) NS(Track_sixtrack_sequ_by_sequ_cbuffer)(
    SIXTRL_CBUFFER_ARGPTR_DEC NS(CBuffer)* SIXTRL_RESTRICT track_pbuffer,
    SIXTRL_CBUFFER_ARGPTR_DEC const NS(CBuffer) *const cmp_pbuffer,
    SIXTRL_CBUFFER_ARGPTR_DEC const NS(CBuffer) *const lattice_buffer,
    SIXTRL_CBUFFER_ARGPTR_DEC NS(CBuffer)* SIXTRL_RESTRICT diff_buffer,
    ...
    NS(status_type) status = ( NS(status_type) )STATUS_GENERAL_FAILURE;
           ...
           NS(CBuffer_capacity)( track_pbuffer ) ) )
    {
        ...
        status = NS(Track_sixtrack_sequ_by_sequ_cobj_flat_buffer)(
...
#endif /* !defined( _GPUCODE ) && defined( __cplusplus ) */
#endif /* SIXTRACKLIB_TESTLIB_COMMON_TRACK_TEST_CMP_SIXTRACK_H__ */
