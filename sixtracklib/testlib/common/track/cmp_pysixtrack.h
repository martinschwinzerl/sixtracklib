#ifndef SIXTRACKLIB_TESTLIB_COMMON_TRACK_TEST_CMP_PYSIXTRACK_H__
#define SIXTRACKLIB_TESTLIB_COMMON_TRACK_TEST_CMP_PYSIXTRACK_H__
...
NS(Track_testlib_cmp_elem_by_elem_pysixtrack_cobj_flat_buffer)(
...
NS(Track_testlib_cmp_elem_by_elem_sixtrack_cbuffer)(
    ...
    NS(cobj_size_type) const slot_size )
{
        ...
        (  NS(CObjFlatBuffer_num_objects)( lattice_buffer, slot_size ) > 0u ) )
    {
        ...
        NS(cobj_size_type) first_fail_idx = NUM_SEQUENCES + 1u;
        ...
        for( ; ii < NUM_SEQUENCES ; ++ii )
        {
            ...
            for( ; jj < NUM_PARTICLES ; ++jj )
            {
                ...
                if( diff_pset != SIXTRL_NULLPTR )
                {
                    NS(Particle_difference)( &cmp_p, &p, &diff );
                ...
                if( ( status != ( NS(status_type) )SIXTRL_STATUS_SUCCESS ) ||
                    ( 0 != NS(Particle_compare_with_tolerances)(
            ...
            if( 0 != NS(Particles_compare_with_tolerances)(
                    pset_b, pset_track, rel_tolerance, abs_tolerance ) )
            {
                first_fail_idx = ii;
                status = ( NS(status_type) )SIXTRL_STATUS_GENERAL_FAILURE;
                break;
        ...
        if( ptr_first_fail_idx != SIXTRL_NULLPTR )
        {
            *ptr_first_fail_idx = first_fail_idx;
        }
    }
...
#endif /* !defined( _GPUCODE ) && defined( __cplusplus ) */
#endif /* SIXTRACKLIB_TESTLIB_COMMON_TRACK_TEST_CMP_PYSIXTRACK_H__ */
