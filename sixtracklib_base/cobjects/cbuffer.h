#ifndef SIXTRACKLIB_BASE_COBJECTS_CBUFFER_H__
#define SIXTRACKLIB_BASE_COBJECTS_CBUFFER_H__
...
{
    class SIXTRL_BASE_EXPORT_API CBuffer :
        public SIXTRL_CXX_NAMESPACE::CBufferView
    {
        private:
            ...
            if( elem != SIXTRL_NULLPTR )
            {
                ...
                if( !can_add_copy )
                {
                    status_t status = ( this->capacity() >= requ_buffer_size )
                        ? st::STATUS_SUCCESS : this->reserve( requ_buffer_size );
                  ...
                  bool const init_with_zeros = false )
        {
            ...
            base_t::status_t status = st::STATUS_GENERAL_FAILURE;
            if( elem != SIXTRL_NULLPTR )
            {
                ...
                status =( st::ObjData_type_conversion_can_requ_prepare<
                    ...
                    ? conv_t::prepare( &temp, elem ) : st::STATUS_SUCCESS;
...
extern "C" { typedef SIXTRL_CXX_NAMESPACE::CBuffer NS(CBuffer); }
...
#endif /* C++, Host */
#endif /* SIXTRACKLIB_BASE_COBJECTS_CBUFFER_H__ */
